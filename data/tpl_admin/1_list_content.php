...
<div class="tips">
	...
	<div class="action"><a href="javascript:$.phpok_list.set(<?php echo $pid;?>);void(0);"><?php echo P_Lang("编辑项目");?></a></div>
	<?php } ?>
	<?php if($popedom['add']){ ?>
	...
	<div style="float:left;width:800px;">
		...
		<tr>
			...
			<td><select name="attr" id="search_attr">
				...
				<option value="<?php echo $key;?>"<?php if($attr && is_array($attr) && in_array($key,$attr)){ ?> selected<?php } ?>><?php echo $value;?></option>
				<?php } ?>
			</select></td>
			<?php } ?>
		...
		<tr>
			<td><select name="orderby_search">
				...
				<option value="id_max"<?php if($orderby_search=='id_max'){ ?> selected<?php } ?>><?php echo P_Lang("ID值从大到小");?></option>
				<option value="id_min"<?php if($orderby_search=='id_min'){ ?> selected<?php } ?>><?php echo P_Lang("ID值从小到大");?></option>
			</select></td>
...
<tr id="list_<?php echo $value['id'];?>" title="<?php echo $rs['alias_title'] ? $rs['alias_title'] : P_Lang('主题');?>: <?php echo $value['title'];?>&#10;<?php echo P_Lang("发布日期");?>: <?php echo date('Y-m-d H:i:s',$value['dateline']);?>">
	...
	<td>
		<div class="button-group">
			...
			<?php if($popedom['comment'] && $rs['comment_status'] && $comments[$value['id']]){ ?><input type="button" class="phpok-btn" value="<?php echo P_Lang("评论");?>" onclick="$.admin_list.reply_it('<?php echo $value['id'];?>')"<?php if($comments[$value['id']]['uncheck']){ ?> style="color:red;" title="<?php echo P_Lang("未审核评论：");?><?php echo $comments[$value['id']]['uncheck'];?>"<?php } ?> /><?php } ?>
	...
	<tr id="list_<?php echo $vv['id'];?>" title="<?php echo $rs['alias_title'] ? $rs['alias_title'] : '主题';?>：<?php echo $vv['title'];?>&#10;发布日期：<?php echo date('Y-m-d H:i:s',$vv['dateline']);?>">
		...
		<td>
			<div class="button-group">
				...
				<?php if($popedom['comment'] && $rs['comment_status'] && $comments[$vv['id']]){ ?><input type="button" class="phpok-btn" value="<?php echo P_Lang("评论");?>" onclick="$.phpok_list.reply_it('<?php echo $vv['id'];?>')"<?php if($comments[$vv['id']]['uncheck']){ ?> style="color:red;" title="<?php echo P_Lang("未审核评论：");?><?php echo $comments[$vv['id']]['uncheck'];?>"<?php } ?> /><?php } ?>
			</div>
		</td>
	</tr>
	<?php } ?>
