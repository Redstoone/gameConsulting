...
<div class="tips">
	...
	<a href="<?php echo admin_url('cate');?>"><?php echo P_Lang("栏目管理");?></a>
	<?php if($id){ ?>
	&raquo; <?php echo P_Lang("编辑分类信息");?>
	<?php } else { ?>
	&raquo; <?php echo P_Lang("添加新分类（添加字段前建议先保存数据，每添加完一个字段系统会自动刷新）");?>
	<?php } ?>
</div>
<ul class="group">
...
<div class="table">
	<div class="title">
		<?php echo P_Lang("标识：");?>
		<span class="note"><?php echo P_Lang("限");?><span class="red"><?php echo P_Lang("字母、数字、下划线或中划线且必须是字母开头");?></span></span>
	</div>
	<div class="content">
...
<div class="table">
	<div class="title">
		<?php echo P_Lang("上级分类：");?>
		<span class="note"><?php echo P_Lang("请选择上一级分类信息");?></span>
	</div>
	<div class="content">
		<select name="parent_id" id="parent_id" class="p_cate">
			<option value="0"><?php echo P_Lang("设置为根分类");?></option>
			<?php $parentlist_id["num"] = 0;$parentlist=is_array($parentlist) ? $parentlist : array();$parentlist_id = array();$parentlist_id["total"] = count($parentlist);$parentlist_id["index"] = -1;foreach($parentlist as $key=>$value){ $parentlist_id["num"]++;$parentlist_id["index"]++; ?>
...
<div class="table">
	<div class="content">
		<select id="_tmp_select_add" style="padding:3px">
			<option value=""><?php echo P_Lang("请选择扩展字段…");?></option>
		...
		<input type="button" value="<?php echo P_Lang("添加");?>" onclick="_update_select_add()" class="button2" />
...
function _update_select_add()
{
	...
	if(!val){
		$.dialog.alert('<?php echo P_Lang("请选择要添加的扩展");?>');
		return false;
	}
...
<div id="admin_setting" class="hide">
	<div class="table">
		<div class="title">
			...
			<span class="note"><?php echo P_Lang("启用此项将替换项目中的设置");?></span>
		</div>
		<div class="content">
	...
	<div class="table">
		<div class="title">
			...
			<span class="note"><?php echo P_Lang("此处设置自定义模板，将替代项目中的模板设置");?></span>
		</div>
		<div class="content">
	...
	<div class="table">
		<div class="title">
			<?php echo P_Lang("默认子分类扩展字段：");?>
			<span class="note"><?php echo P_Lang("此项用于配置默认的子项扩展字段属性，留空表示不添加子分类扩展字段");?></span>
		</div>
		<div class="content">
			<ul class="layout">
				...
				<li><label><input type="checkbox" name="_extfields[]" value="<?php echo $key;?>"<?php if($ext2 && in_array($key,$ext2)){ ?> checked<?php } ?> /><?php echo $value['title'];?></label></li>
				<?php } ?>
			</ul>
			<div class="clear"></div>
		</div>
