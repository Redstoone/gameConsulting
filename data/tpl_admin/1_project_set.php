...
<div class="table">
	...
	<div class="content">
		<select id="module" name="module" onchange="$.admin_project.module_change(this)">
...
<div id="module_set2" class="hidden">
	<div class="table">
		...
		<div class="content">
			<input type="text" id="psize2" name="psize2" value="<?php echo $rs['psize'] ? $rs['psize'] : 30;?>" class="short" />
		</div>
	</div>
	...
	<div class="table">
		...
		<div class="content">
			<input type="text" id="orderby2" name="orderby2" class="long" value="<?php echo $rs['orderby'] ? $rs['orderby'] : 'id DESC';?>" />
			<input type="button" value="<?php echo P_Lang("清空");?>" class="phpok-btn" onclick="$('input[name=orderby2]').val('')" />
			<div style="display:block;margin-top:5px;">
				<div class="button-group" id="tmp_orderby_btn2">
					<input type="button" value="ID" onclick="phpok_admin_orderby('orderby2','id')" class="phpok-btn" />
				</div>
			</div>
		</div>
		<div class="clear"></div>
...
<div id="module_set" class="hidden">
	...
	<div class="table">
		<div class="title">
			<?php echo P_Lang("运费模板");?>：
			<span class="note"><?php echo P_Lang("请选择这个项目的运费计算方式");?></span>
		</div>
		<div class="content">
	...
	<div class="table">
		<div class="title">
			<?php echo P_Lang("自定义产品属性：");?>
			<span class="note"><?php echo P_Lang("启用此项后，将允许设定可选产品的一些属性，如颜色，款式，支持价格浮动");?></span>
		</div>
		<div class="content">
			<ul class="layout">
	...
	<input type="hidden" data-name="emailtpl" value="<?php echo $value['identifier'];?>" data-title="<?php echo $value['title'];?>" data-note="<?php echo $value['note'];?>" data-orderby="<?php echo $tmpid['num'];?>" />
	<?php } ?>
	...
	<div class="table">
		...
		<div class="content">
			<input type="text" name="etpl_admin" id="etpl_admin" value="<?php echo $rs['etpl_admin'];?>" class="long"<?php if(!$rs['etpl_admin']){ ?> placeholder="不通知"<?php } ?> />
		</div>
	</div>
	<div class="table">
		<div class="title">
		...
		<div class="content">
			<input type="text" name="etpl_user" id="etpl_user" value="<?php echo $rs['etpl_user'];?>" class="long"<?php if(!$rs['etpl_user']){ ?> placeholder="不通知"<?php } ?> />
		</div>
	</div>
	</div>
	...
	<div class="table">
		...
		<div class="content">
			<input type="text" name="etpl_comment_admin" id="etpl_comment_admin" value="<?php echo $rs['etpl_comment_admin'];?>" class="long"<?php if(!$rs['etpl_comment_admin']){ ?> placeholder="不通知"<?php } ?> />
		</div>
	</div>
	<div class="table">
		<div class="title">
		...
		<div class="content">
			<input type="text" name="etpl_comment_user" id="etpl_comment_user" value="<?php echo $rs['etpl_comment_user'];?>" class="long"<?php if(!$rs['etpl_comment_user']){ ?> placeholder="不通知"<?php } ?> />
		</div>
	</div>
	</div>
...
<div class="table">
	...
	<div class="content">
		<ul class="layout">
			<?php $icolist_id["num"] = 0;$icolist=is_array($icolist) ? $icolist : array();$icolist_id = array();$icolist_id["total"] = count($icolist);$icolist_id["index"] = -1;foreach($icolist as $key=>$value){ $icolist_id["num"]++;$icolist_id["index"]++; ?>
