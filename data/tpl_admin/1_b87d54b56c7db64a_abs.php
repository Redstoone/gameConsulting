	...
	<div class="_select">
		...
		<tr>
			...
			<td valign="top">
				...
				<?php $tmpid["num"] = 0;$_catelist=is_array($_catelist) ? $_catelist : array();$tmpid = array();$tmpid["total"] = count($_catelist);$tmpid["index"] = -1;foreach($_catelist as $key=>$value){ $tmpid["num"]++;$tmpid["index"]++; ?>
				<option value="<?php echo $value['id'];?>"<?php if($value['id'] == $_rs['cate_id']){ ?> selected<?php } ?>><?php echo $value['title'];?></option>
				<?php } ?>
				</select>
			</td>
			<td valign="top"><button onclick="$.phpokform.upload_cate_create('<?php echo $_rs['identifier'];?>','<?php echo $_rs['upload_type']['title'];?>','<?php echo $_rs['upload_type']['ext'];?>')" class="button" type="button" title="<?php echo P_Lang("添加新分类");?>">+</button></td>
			<?php } ?>
	...
	$("#select_res_<?php echo $_rs['identifier'];?>").click(function(){
		...
		if(t == 'true'){
			...
			$.dialog.open(url,{
				...
				'width': width,
				'height': height,
				'ok': true,'okVal':'<?php echo P_Lang("关闭");?>'
			});
		}else{
			if(old){
				url += "&selected="+old;
